...
const MOCK_SLOTS: MockSlot[] = [
  {
    slot_index: 1,
    medicine_id: "mock-med-001",
    meal_times: ["morning"],
    dose_quantity: 1,
    medicineName: "แอมโลดิปีน 5mg",
  },
  {
    slot_index: 2,
    medicine_id: "mock-med-002",
    meal_times: ["morning", "noon"],
    dose_quantity: 1,
    medicineName: "เมทฟอร์มิน 500mg",
  },
  {
    slot_index: 3,
    medicine_id: "mock-med-003",
    meal_times: ["evening", "bedtime"],
    dose_quantity: 1,
    medicineName: "แอสไพริน 81mg",
  },
];
...
export default function DispenseRunScreen() {
  ...
  const patientName =
    typeof params.patientName === "string" ? params.patientName : "Patient";
  ...
  const [slots, setSlots] = useState<
    Pick<
      DispenserSlotsRow,
      "slot_index" | "medicine_id" | "meal_times" | "dose_quantity"
    >[]
  >([]);
  ...
  useEffect(() => {
    ...
    const meals = MEAL_ORDER.filter((m) =>
      slots.some((s) => s.meal_times.includes(m)),
    );
  ...
  const runForCell = async (day: number, mealIdx: number) => {
    ...
    try {
      ...
      onProgress({
        type: "error",
        message: err instanceof Error ? err.message : "Dispense failed",
      });
      setStopped(true);
    } finally {
      setBusy(false);
  ...
  const advanceFrom = (day: number, mealIdx: number) => {
    ...
    setCellStatuses((prev) => ({
      ...prev,
      [cellKey(nextD, nextM)]: "dispensing",
    }));
  ...
  const handleReady = () => {
    ...
    setCellStatuses((prev) => ({
      ...prev,
      [cellKey(active.d, active.m)]: "done",
    }));
      ...
      <SafeAreaView className="flex-1 bg-[#F7F2EA] px-6">
        ...
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="text-sm text-[#7D6E60] text-center mb-8">
            This session has no confirmed slots — go back and load the cabinet
            first.
          </Text>
          <TouchableOpacity
            onPress={() => router.back()}
    ...
    <View className="flex-1 bg-[#F7F2EA]">
      ...
      <SafeAreaView className="flex-1" edges={["top", "left", "right"]}>
        ...
        <View className="px-5 pt-3 pb-3 flex-row items-center">
          <Pressable
            onPress={() => router.back()}
            className="w-10 h-20 items-center justify-center -ml-2"
          >
          ...
          <View className="flex-1 ml-1">
            ...
            <Text
              className="text-[20px] leading-[26px] font-bold text-[#2E241B] mt-0.5"
              numberOfLines={1}
            >
        ...
        <View className="flex-row mb-3 px-4 gap-1">
          {DAYS.map((label, i) => {
            ...
            return (
              ...
              >
                <Text
                  className={`text-[10px] font-semibold ${isView ? "text-white" : "text-[#5A5145]"}`}
                >
                  {shortLabel}
                </Text>
                {dayHasActive && !isView ? (
                  <View className="w-1 h-1 rounded-full bg-[#C96B1A] mt-0.5" />
                ) : null}
              </TouchableOpacity>
            );
          })}
        ...
        <View className="px-5 pb-2 flex-row items-center">
          <Text className="text-[18px] font-bold text-[#C96B1A]">
            {FULL_DAYS[viewDayIndex]}
          </Text>
          {active?.d === viewDayIndex ? (
            <Text className="text-[12px] text-[#C96B1A] ml-2">
              — dispensing now
            </Text>
          ) : null}
        </View>
        ...
        >
          ...
          <View className="bg-white rounded-2xl border border-[#EADBCB] p-4 mt-2">
            ...
            >
              {events.length === 0 ? (
                <Text className="text-[12px] text-[#9B8E80] italic">
                  No activity yet…
                </Text>
              ) : (
                events.map((ev, i) => (
                  <View key={i} className="flex-row items-start mb-1.5">
                    ...
                    <Text className="text-[12px] text-[#2E241B] flex-1">
                      {ev.message}
                    </Text>
                  </View>
                ))
...
function MealCard({
  ...
  const cardStyle = isDone
    ? { backgroundColor: "#E3FCEA", borderColor: "#A8E5C9" }
    : isActive
      ? { backgroundColor: "#FFFFFF", borderColor: "#C96B1A" }
          ...
          meds.map((m) => (
            <View key={m!.slot_index} className="flex-row items-center py-1">
              <View className="w-7 h-7 rounded-lg bg-[#FFF1DD] items-center justify-center mr-2">
                <Text className="text-[10px] font-bold text-[#8E4B14]">
                  #{m!.slot_index}
                </Text>
              </View>
              <Text
                className="flex-1 text-[12px] text-[#2E241B]"
                numberOfLines={1}
              >
                Slot {m!.slot_index}
              </Text>
              <Text className="text-[11px] text-[#7D6E60]">
                ×{m!.dose_quantity}
              </Text>
            </View>
          ))

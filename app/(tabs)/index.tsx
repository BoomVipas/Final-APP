...
import { type AlertCardData, type DispensePatientCard, type WardFilterOption } from '../../src/components/home/types'
...
function formatWardLabel(wardId: string | null | undefined): string {
  ...
  if (digitMatch) {
    ...
    if (index >= 1 && index <= 26) return `Ward ${String.fromCharCode(64 + index)}`
  }
...
const demoAlertCards: AlertCardData[] = [
  { id: 'demo-alert-1', patientName: 'Mr. Somchai', title: 'Medication running low', medication: 'Risperidone 2 mg, 4 tablets remaining', detail: 'Runs out on March 14', footnote: 'Notify 3 days in advance', cta: 'Notify 3 days in advance', ctaTone: 'danger' },
  { id: 'demo-alert-2', patientName: 'Mr. Polo',    title: 'Scheduled medication',   medication: 'Metformin 500 mg', detail: 'to be taken at 12:00 regularly', footnote: 'Next dose in 2 hours 18 minutes', cta: 'Next dose in 2 hours 18 minutes', ctaTone: 'danger' },
]

const demoPatientCards: DispensePatientCard[] = [
  { id: 'p1', name: 'Mr. Somchai Wongsri', room: 'Room A-102', age: '78', wardId: 'ward-a', ward: 'Ward A', tablets: '12 tablets', statusLabel: 'Urgent',    statusTone: 'urgent',  tags: ['• Risperidone 2 mg', '• Take with food', '• Metoprolol'], moreCount: 3 },
  { id: 'p2', name: 'Mrs. Polo Suksan',    room: 'Room B-201', age: '81', wardId: 'ward-b', ward: 'Ward B', tablets: '9 tablets',  statusLabel: 'Pending',   statusTone: 'pending', tags: ['• Amlodipine 5 mg (new)', '• Losartan 50 mg'], note: '⚠ Medication has been changed - please check before dispensing', moreCount: 4 },
  { id: 'p3', name: 'Mr. Mana Jai',        room: 'Room B-203', age: '69', wardId: 'ward-b', ward: 'Ward B', tablets: '5 tablets',  statusLabel: 'Dispensed', statusTone: 'done',    tags: ['• Dementia medication', '• Aspirin 81 mg'] },
]
...
export default function HomeScreen() {
  ...
  const { scheduleGroups, pendingCount, completedCount, fetchSchedule, skipDose, subscribeToRealtime } = useMedicationStore()
  ...
  const fetchWardOptions = useCallback(async (scopeWardId: string) => {
    try {
      let patientQuery = supabase.from('patients').select('ward_id').eq('status', 'active').not('ward_id', 'is', null)
      if (scopeWardId) patientQuery = patientQuery.eq('ward_id', scopeWardId)

      const { data: patientRows, error } = await patientQuery
      ...
      if (wardIds.length === 0) { setWardOptions([]); setWardLabelById({}); return }

      const { data: wards } = await supabase.from('wards').select('id, name, floor').in('id', wardIds)
      const wardsById = new Map((wards ?? []).map((w) => [w.id, w as Pick<WardsRow, 'name' | 'floor'>]))
      const labels = Object.fromEntries(wardIds.map((id) => [id, formatWardOptionLabel(id, wardsById.get(id))]))

      setWardLabelById(labels)
      setWardOptions(wardIds.map((id) => ({ id, label: labels[id], patientCount: counts.get(id) ?? 0 })).filter((o) => o.patientCount > 0).sort((a, b) => a.label.localeCompare(b.label)))
    } catch {
      if (scopeWardId) {
  ...
  const loadData = useCallback(async () => {
    if (!user) return
    if (USE_MOCK) { setPending(MOCK_HANDOVER as unknown as ShiftHandoversRow) }
    else if (wardScope) { fetchPending(wardScope) }
    await Promise.all([
  ...
  useEffect(() => {
    ...
    if (!wardScope) { realtimeUnsubRef.current = null; return }
    realtimeUnsubRef.current = subscribeToRealtime(wardScope, todayStr)
    return () => { realtimeUnsubRef.current?.(); realtimeUnsubRef.current = null }
  }, [subscribeToRealtime, todayStr, wardScope])

  useEffect(() => {
    if (wardOptions.length === 1) { setSelectedWardFilter(wardOptions[0].id); return }
    if (selectedWardFilter !== 'all' && !wardOptions.some((o) => o.id === selectedWardFilter)) setSelectedWardFilter('all')
  ...
  const liveAlertCards = useMemo<AlertCardData[]>(() => {
    ...
    return activeAlerts.slice(0, 2).map((alert, i) => ({
      id: alert.id,
      patientName: alert.patient_id ? patientNameById.get(alert.patient_id) ?? `Patient ${i + 1}` : `Patient ${i + 1}`,
  ...
  const livePatientCards = useMemo<DispensePatientCard[]>(() => {
    ...
    }).filter((p) => p.tags.length > 0).sort((a, b) => ({ urgent: 0, pending: 1, done: 2 }[a.statusTone] - { urgent: 0, pending: 1, done: 2 }[b.statusTone])).slice(0, 3)
  ...
  const filteredPatientCards = patientCards.filter((p) => selectedWardFilter === 'all' || p.wardId === selectedWardFilter)
  ...
  const openNotifications = (filter?: 'all' | 'stock') =>
    router.push({ pathname: '/notifications', params: filter && filter !== 'all' ? { filter } : undefined })

  const openPatientDetail = (patientId: string) => router.push(`/patient/${patientId}`)

  const showPatientActions = (patient: DispensePatientCard) => {
    const pendingItems = allTodayItems.filter((item) => item.patient_id === patient.id && item.status === 'pending' && !item.conflict_flag)
    Alert.alert(patient.name, 'Choose the next workflow for this patient.', [
      ...
      {
        text: pendingItems.length > 0 ? `Skip ${pendingItems.length} pending` : 'Skip (no pending)',
        style: 'destructive',
        onPress: () => {
          if (!user || pendingItems.length === 0) return
          Alert.alert('Skip pending doses?', `Mark all ${pendingItems.length} of today's pending doses for ${patient.name} as skipped?`, [
            { text: 'Cancel', style: 'cancel' },
            {
              text: 'Skip all', style: 'destructive',
              onPress: async () => {
                let skipped = 0; let failed = 0
                for (const item of pendingItems) {
                  try { await skipDose(item, user.id); skipped++ } catch { failed++ }
                }
                await loadData()
                Alert.alert('Skipped', failed > 0 ? `Skipped ${skipped}; ${failed} failed.` : `Skipped ${skipped} dose${skipped === 1 ? '' : 's'}.`)
              },
            },
          ])
        },
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FBF0E3' }}>
        ...
        >
          ...
          >
            ...
            <View className="mt-5 px-3 flex-row gap-3">
              <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 18, padding: 6, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 12, shadowOffset: { width: 0, height: 3 }, elevation: 3 }}>
                <StatCard label="Total Recipients" value={totalRecipients} SvgIcon={PillIcon} iconBg="#FFFFFF" iconColor="#2E2C2A" gradient={{ colors: ['#FFFFFF', '#F1F1F1'], start: { x: 0, y: 1 }, end: { x: 0, y: 0 } }} onPress={() => router.push('/patients')} />
              </View>
              <View style={{ flex: 1, backgroundColor: 'white', borderRadius: 18, padding: 6, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 12, shadowOffset: { width: 0, height: 3 }, elevation: 3 }}>
                <StatCard label="Distributed Today" value={distributedToday} SvgIcon={HourglassIcon} iconBg="#FFFFFF" iconColor="#1B8C67" gradient={{ colors: ['#E3FCEA', '#E3FCEA00'], start: { x: 0, y: 0 }, end: { x: 0, y: 1 } }} onPress={() => router.push('/schedule')} />
              </View>
            </View>
          </LinearGradient>
          ...
          <View style={{ marginTop: -36, marginHorizontal: 16, marginBottom: -12 }}>
            <LinearGradient colors={['#FFFFFF', '#FFE6E6']} start={{ x: 1, y: 0 }} end={{ x: 0, y: 0 }} style={{ borderRadius: 14, borderWidth: 1, borderColor: '#F1F1F1', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 20, shadowOffset: { width: 0, height: 6 }, elevation: 4 }}>
              <TouchableOpacity onPress={() => router.push('/schedule')} activeOpacity={0.88} style={{ borderRadius: 14, paddingHorizontal: 14, paddingVertical: 4, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {pendingHandover ? (
            <View className="px-4 pt-4">
              <TouchableOpacity onPress={() => router.push('/handover')} activeOpacity={0.9} style={{ flexDirection: 'row', backgroundColor: '#FFFFFF', borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: '#EADBCB', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 10, shadowOffset: { width: 0, height: 3 }, elevation: 2 }}>
                ...
                <View style={{ flex: 1, paddingHorizontal: 14, paddingVertical: 14, flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 11, fontWeight: '700', color: '#8E4B14', letterSpacing: 1, textTransform: 'uppercase' }}>
                      {(() => { const h = new Date(pendingHandover.shift_start).getHours(); return h >= 6 && h < 14 ? 'เวรเช้า / Morning shift' : h >= 14 && h < 22 ? 'เวรบ่าย / Afternoon shift' : 'เวรดึก / Night shift' })()}
                    </Text>
                    <Text style={{ fontSize: 14, fontWeight: '700', color: '#2E241B', marginTop: 2 }}>ยืนยันการรับเวร / Acknowledge handover</Text>
                    {(() => {
                      ...
                      return pending === 0 ? null : <Text style={{ fontSize: 12, color: '#A3322A', marginTop: 4 }}>🔴 {pending} ยาค้าง / pending dose{pending > 1 ? 's' : ''}</Text>
                    })()}
                  </View>
          ...
          <View className="px-6 pt-6">
            ...
            <View className="bg-[#FFFDF9] rounded-[18px] p-4 shadow-sm mb-6">
              <View className="flex-row items-center justify-between mb-4">
                ...
                <TouchableOpacity onPress={() => setAlertsExpanded((c) => !c)} className="w-8 h-8 rounded-full bg-white border border-[#EFE6DB] items-center justify-center">
                  <Ionicons name={alertsExpanded ? 'chevron-up' : 'chevron-down'} size={16} color="#484440" />
                </TouchableOpacity>
              </View>
              {visibleAlertCards.map((alert) => (
                <AlertCard key={alert.id} alert={alert} onPress={() => openNotifications('all')} onMorePress={() => showAlertActions(alert)} />
              ))}
          ...
          <View className="bg-white pt-6 pb-4">
            <View className="px-6">
              ...
              {filteredPatientCards.map((patient) => (
                <PatientCard key={patient.id} patient={patient} onPress={() => openPatientDetail(patient.id)} onMorePress={() => showPatientActions(patient)} />
              ))}
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.85}
        style={{ position: 'absolute', right: 16, bottom: Math.max(96, insets.bottom + 80), width: 56, height: 56, borderRadius: 28, backgroundColor: '#E8721A', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOpacity: 0.25, shadowRadius: 6, shadowOffset: { width: 0, height: 3 }, elevation: 6 }}
      >
        <Ionicons name="mic" size={26} color="#fff" />
      </TouchableOpacity>

      <BottomNav activeTab="home" onHome={() => router.replace('/(tabs)')} onWard={() => router.replace('/(tabs)/patients')} onProfile={() => router.replace('/(tabs)/settings')} />
    </View>
  )
}
